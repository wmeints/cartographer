...
public static partial class OperatorLog
{
    [LoggerMessage(
        ...
        Message = "Creating deployment {DeploymentName} for workspace {WorkspaceName} in namespace {NamespaceName}"
    )]
    ...
    [LoggerMessage(
        EventId = 1005,
        Level = LogLevel.Information,
        Message = "Scaling deployment {DeploymentName} for workspace {WorkspaceName} in namespace {NamespaceName} to {Replicas} replicas"
    )]
